...
pub async fn download_and_open_release_asset(
    ...
    download_to_path(&url, &temp_path).await?;
    ...
    app_handle
        .opener()
        .open_path(target_path.to_string_lossy().into_owned(), None::<String>)
...
async fn download_to_path(url: &str, target_path: &Path) -> Result<(), String> {
