  ...
  const showPromptMenu = async (
    ...
    const menu = await Menu.new({
      items: [
        ...
        await MenuItem.new({
          text: `移动到${nextScope === "workspace" ? "项目" : "通用"}`,
  ...
  return (
    <PanelShell
      ...
      headerRight={
        <PanelMeta className="prompt-panel-meta">
          {hasPrompts ? `${totalCount} 个 Prompt` : "没有 Prompt"}
        </PanelMeta>
      }
      search={
        <PanelSearchField
          ...
          placeholder="过滤 Prompt"
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          aria-label="过滤 Prompt"
    ...
    >
      <div className="prompt-panel-scroll">
          ...
          {workspacePrompts.length > 0 ? (
            ...
            <div className="prompt-empty-card">
              ...
              <div className="prompt-empty-text">
                <div className="prompt-empty-title">还没有项目 Prompt</div>
        ...
        <div className="prompt-section">
          <div className="prompt-section-header">
            <div className="prompt-section-title">通用 Prompt</div>
            <button
              type="button"
              className="ghost icon-button prompt-section-add"
              onClick={() => startCreate("global")}
              aria-label="添加通用 Prompt"
              title="添加通用 Prompt"
            >
              <Plus aria-hidden />
            </button>
          ...
          {globalPrompts.length > 0 ? (
            ...
            <div className="prompt-empty-card">
              <ScrollText className="prompt-empty-icon" aria-hidden />
              <div className="prompt-empty-text">
                <div className="prompt-empty-title">还没有通用 Prompt</div>
                <div className="prompt-empty-subtitle">
                  在这里创建，或把 .md 文件放入{" "}
                  {canRevealGeneralPrompts ? (
                    <button
