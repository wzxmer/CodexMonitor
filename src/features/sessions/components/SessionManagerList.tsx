  ...
  return (
    <div className="session-manager-list">
      ...
      {props.sessions.length === 0 ? <div className="session-manager-state">{t("sessionManager.empty")}</div> : props.sessions.map((session) => (
        <SessionManagerRow key={session.key} session={session} source={sourceById.get(session.sourceId)} selected={props.selected.has(session.key)} focused={props.focusedKey === session.key} compact={props.compact} resuming={props.resumingKey === session.key} archiving={props.archivingKeys.has(session.key)} deleting={props.deletingKeys?.has(session.key) ?? false} onToggleSelected={() => props.onToggleSelected(session.key)} onFocus={props.onFocus ? () => props.onFocus?.(session) : undefined} onResume={() => props.onResume(session)} onArchive={() => props.onArchive(session)} onDerive={() => props.onDerive(session)} onPermanentDelete={() => props.onPermanentDelete?.(session)} />
      ))}
