...
export function SessionManagerRow({ session, source, selected, resuming, archiving, deleting, onToggleSelected, onResume, onArchive, onDerive, onPermanentDelete }: Props) {
  const { t } = useI18n();
  return (
    <div className={`session-manager-row${selected ? " is-selected" : ""}`}>
      ...
      <button type="button" className="session-manager-row-content" onClick={onResume} disabled={resuming || !session.projectExists}>
        ...
        {session.preview && <span className="session-manager-row-preview">{session.preview}</span>}
      ...
      <span className="session-manager-row-actions">
