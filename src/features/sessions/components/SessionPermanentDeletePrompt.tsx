...
type Props = { session: ManagedSession; source?: SessionSource; childCount: number; busy: boolean; onCancel: () => void; onConfirm: (cascade: boolean) => void };

export function SessionPermanentDeletePrompt({ session, source, childCount, busy, onCancel, onConfirm }: Props) {
  const { t } = useI18n();
  ...
  return <ModalShell className="session-delete-modal" onBackdropClick={busy ? undefined : onCancel} ariaLabel={t("sessionManager.permanentDeleteTitle")}>
    ...
    <div>{source?.name ?? session.sourceId}</div><div>{session.threadId}</div><div>{session.archivedAt ? new Date(session.archivedAt).toLocaleString() : "—"}</div>
