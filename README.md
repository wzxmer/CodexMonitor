...
Windows 请继续使用与当前安装相同的格式更新：`.exe` 对应 NSIS，`.msi` 对应 MSI。应用检测到两种格式共存或安装器归属不明时会停止自动更新；若旧 NSIS 与 MSI 共用安装目录，请勿运行旧卸载器或直接删除共享目录，应先备份并修复安装器归属。
...
Keep using the same Windows installer family for updates: `.exe` for NSIS and `.msi` for MSI. Automatic updates stop when both families are registered or installer ownership is unclear. If a stale NSIS entry shares the MSI install directory, do not run the old uninstaller or delete the shared directory; back up your data and repair installer ownership first.
